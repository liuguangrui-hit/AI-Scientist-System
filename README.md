...
每个研究 idea 拆解为一棵由实验检验的假设树，不同 idea 共享的假设把树连成森林。森林的演化由两个过程共同驱动：

| | 方法 | 作用 |
| --- | --- | --- |
| 展开 | 苏格拉底式追问 | 多个 agent 对假设层层追问，拆解为可由实验检验的子假设。未通过检验的假设继续追问拆解。 |
| 收敛 | 奥卡姆剃刀 | 强化学习将多条具体假设抽象为一条更普适的假设，并剪除冗余分支。 |

**整体目标：以最精简的假设，解释最多的实验证据。**
...
| 操作 | 系统行为 |
| --- | --- |
| 提交实验 | 实验依次经过排队，分配资源，运行，直至完成。完成后将带符号的证据写入对应假设，所有引用该假设的课题同步更新。模拟时钟独立推进，离开页面期间队列照常消化。 |
...
| 浏览假设全景 | 支持缩放和平移，节点可拖动；共享假设位于引用它的各课题之间。 |
...
系统读取以下文件：`tree.json`（课题，假设与假设树） `experiments.jsonl` `events.jsonl` `index.jsonl`（文献）
...
`tree.json` `index.jsonl` `experiments.jsonl` 和 `artifacts/` 由各 agent 维护。涉及这些文件的操作会被拒绝并给出原因；
...
| 文档 | 内容 |
| --- | --- |
| [docs/CONCEPTS.zh-CN.md](docs/CONCEPTS.zh-CN.md) | 森林模型：共享假设，带符号证据，以及裁定的传播 |
...
包含数十个 idea，近千条假设和两千余条带符号证据，呈现共享假设，证据流向以及裁定的传播。无第三方依赖，可直接在浏览器中打开 `index.html`。
...
使用 `main` 分支，以 `npm install` 构建，以 `npm start` 启动，健康检查路径为 `/api/health`。
服务监听 `0.0.0.0:$PORT`，固定使用 `AIS_SOURCE=demo`，每位访问者拥有独立会话。
公开部署仅使用演示数据与模拟实验，不应配置真实项目目录，也不应配置模型密钥或实验执行器。

以 Render Blueprint 连接本仓库后，每次推送至 `main` 均自动部署，部署完成后可检查 `/api/health`。
免费服务在 15 分钟无访问后休眠，再次访问时冷启动约需一分钟；休眠，重启或重新部署都会清空演示记录。
